...
export const GET: APIRoute = async ({ request, locals }) => { try { assertAdmin(request, locals.runtime.env); return json({ articles: await listAdminArticles(locals.runtime.env) }); } catch (e) { if (e instanceof Response) return e; return json({ error: 'failed' }, { status: 500 }); } };
export const POST: APIRoute = async ({ request, locals }) => { try { assertAdmin(request, locals.runtime.env); const body = await request.json() as Body; const result = await upsertArticle(locals.runtime.env, { title: clampText(body.title, 160), dek: clampText(body.dek, 300), body: clampText(body.body, 200000), category: clampText(body.category, 80), tags: Array.isArray(body.tags) ? body.tags.map((t: unknown) => clampText(t, 40)) : [], slug: body.slug, seo: body.seo, status: body.status }); return json({ ok: true, ...result }); } catch (e) { if (e instanceof Response) return e; return json({ error: e instanceof Error ? e.message : 'failed' }, { status: 500 }); } };
